...
define dso_local spir_func <2 x float> @shuffle_vector(<4 x float>) {
  %2 = shufflevector <4 x float> %0, <4 x float> undef, <2 x i32> <i32 0, i32 2>
  ret <2 x float> %2
}
...
define dso_local spir_kernel void @kernel_main(
  ...
  %ov = tail call spir_func <2 x float> @shuffle_vector(<4 x float> %iv)
